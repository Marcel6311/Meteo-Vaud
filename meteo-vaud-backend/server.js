...
async function refreshComets() {
  try {
    const url = "https://ssd-api.jpl.nasa.gov/cad.api" +
      "?comet=true&dist-max=2au&date-min=now&date-max=%2B730&sort=dist&fullname=true";
    const r = await fetch(url);
...
// GET /comets - passages de cometes < 2 UA dans les 2 prochaines annees (JPL SBDB, proxy backend — CORS bloque depuis GitHub Pages)
// Rafraichi toutes les 6h.
...
var httpServer = app.listen(PORT, async () => {
  ...
  setInterval(refreshComets, 6 * 60 * 60 * 1000); // 6h : les trajectoires changent lentement
